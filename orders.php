...
<body>
<button>Orders</button><button>History</button>
...
<ul id="order-list-con">
   ...
   <?php if($_SESSION['identity']=="seller") {
    ...
    <li id="order-list">
        <div id="order-time"><?=$selresult['order_time']?></div>
        ...
        <div id="order-details"><a href="details.php?time=<?=$selresult['order_time']?>&proid=<?=$selresult['pro_id']?>&customerid=<?=$selresult['customer_id']?>" target="_blank">Details</a></div>
    </li>
    <?php }?>
    <!-- for customer -->
   <?php }elseif($_SESSION['identity']=="customer") {
